...
def build_servicedesk_draft_note_prompt(
    ...
    return (
        ...
        "context. Skip a bullet rather than inventing a value. If the "
        "inspection report contains a `### Largest folders` sub-section, "
        "you MAY add 1-3 indented sub-bullets under a single `Largest "
        "folders:` bullet that quote folder path and size from the report. "
        "Do not include subjects, message bodies, attachment names, or any "
        "item-level content.\n"
...
def build_servicedesk_skill_plan_prompt(
    ...
    return (
        ...
        "or none. Allowed values: `exchange.mailbox.inspect`. Do not invent granular "
        "names like `exchange.mailbox.get_properties`; map any such intent to "
