...
def test_dispatches_known_function_and_passes_working_directory(monkeypatch, mock_working_directory):
    ...
    response = call_function_module.call_function(function_call, mock_working_directory)
...
def test_unknown_function_name_returns_error(mock_working_directory):
...
def test_missing_arguments_are_forwarded_as_is(monkeypatch, mock_working_directory):
    mocked = MagicMock(side_effect=mock_dispatched_function)
    monkeypatch.setattr(call_function_module, "get_file_content", mocked)
